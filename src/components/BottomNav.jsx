import { Home, CreditCard, Plus, Heart, MoreHorizontal } from 'lucide-react';
...
const RIGHT_TABS = [
  { id: 'gastos',   label: 'Pareja',   Icon: Heart },
...
function BottomNav({ active, onChange, onAdd }) {
  // "Más" tab is considered active when the current tab is one of the overflow tabs
  const MAS_TABS = ['ingresos', 'archivos', 'chat'];
  const effectiveActive = MAS_TABS.includes(active) ? 'mas' : active;
  ...
  return (
    ...
    >
      <div className="relative flex items-stretch">
        {/* Left tabs */}
        {LEFT_TABS.map(tab => (
          <NavTab key={tab.id} {...tab} active={effectiveActive} onChange={onChange} />
        ))}
